...
export class HubClient {
     ...
     */
    async poll(capabilities, kinds) {
        const res = await this.send('POST', '/api/worker/poll', { machine: this.machine, profiles: capabilities, capabilities, kinds }, this.pollTimeoutMs);
        if (res.status === 204)
            return undefined;
    ...
    async progress(taskId, patch) {
        const res = await this.send('POST', `/api/worker/progress/${taskId}`, patch);
        return (await res.json().catch(() => ({ cancel: false })));
    }
    async result(taskId, report) {
     ...
     */
    async postRaw(path, bytes, contentType, timeoutMs = 60_000) {
     ...
     * 오류 줄로 보인다.
     */
    async send(method, path, payload, timeoutMs) {
        let last;
        for (let attempt = 0; attempt < this.attempts; attempt += 1) {
            if (attempt > 0)
                await sleep(this.retryBaseMs * ((attempt * (attempt + 1)) / 2));
            try {
                const res = await fetch(`${this.base}${path}`, {
                    method,
                    headers: {
                ...
                if (err instanceof HubError && !retryable(err.status))
                    throw err;
                last = err;
            }
        }
